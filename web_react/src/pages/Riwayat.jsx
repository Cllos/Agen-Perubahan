...
import { Filter, ChevronLeft, ChevronRight, Search } from 'lucide-react'; // Pastikan import Search ada
...
export default function Riwayat() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        // Mapping Data dari format Database ke format Aplikasi
        // Database: snake_case (check_in_time, full_name)
        // Aplikasi: camelCase (checkInTime, employeeName)
        const formattedData = data.map(record => ({
          id: record.id,
          date: record.date,
          employeeId: record.employee_id,
          employeeName: record.full_name,
          // Ambil 5 karakter pertama jam (07:55:00 -> 07:55)
          checkInTime: record.check_in_time ? record.check_in_time.substring(0, 5) : '-',
          // Format status agar huruf depan kapital (tepat_waktu -> Tepat Waktu)
  ...
  // Logic Filtering (Tetap sama seperti sebelumnya)
        ...
        <div className="p-6">
          ...
          <div className="mt-6 flex items-center justify-between">
            <p className="text-sm text-gray-600">
              Menampilkan <span className="font-semibold text-gray-900">{filteredRecords.length}</span> data
            </p>
            <button
              onClick={() => {
